...
## Basic options <a name="basic-options"></a>
...
[`-p` option](#paired-reads). Either this or the `-p` option are required. For long lists of read files, they can be set in a
[configuration file](#config-file), to avoid having to type them in on the command line.
...
**`-p, --paired-reads STRING,STRING,...`** <a name="paired-reads"></a> 
...
**`-c, --contigs STRING`** <a name="contigs-file"></a>
