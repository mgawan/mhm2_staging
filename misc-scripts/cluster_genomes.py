...
def write_clusters(bins, prefix):
    clusters = {}
    ...
    #    print(bin, len(cluster))
        SeqIO.write(cluster, prefix + '_' + str(bin) + '.fasta', 'fasta')
...
def test_model(model_data, model):
    try:
        ...
        start_t = time.time()
        prefix = 'cluster_' + model_name
        for fname in glob.glob(prefix + '_*.fasta'):
            os.remove(fname)
        ...
        write_clusters(bins, prefix)
        print('Finished in %.2f s' % (time.time() - start_t))
        if len(clusters) > 1:
            print(prefix)
            eval_clusters.eval_clusters(prefix, run_metaquast=True)
    except:
...
#test_model(model_data_3d, cluster.MeanShift(bandwidth=0.1, max_iter=2000, cluster_all=False))
